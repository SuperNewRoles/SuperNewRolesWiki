var files=[];async function fetch_files()
...
{var suggestions=[];for(var i=0;i<files.length;i++)
{const query_changed=KatakanaToHiragana(query.toLowerCase());const fileSplited=files[i].toLowerCase().split("/");const file=KatakanaToHiragana(fileSplited[fileSplited.length-1]);if(file.includes(query_changed))
...
lastSearchText=search;});document.getElementById("Light-DarkToggleButton").addEventListener("click",function(){document.body.classList.toggle("Dark");if(document.body.classList.contains("Dark")){UpdateStorage("DarkMode","true");}else{UpdateStorage("DarkMode","false");}});document.querySelectorAll('a[href^="#"]').forEach(anchor=>{anchor.addEventListener('click',function(e){e.preventDefault();const targetId=this.getAttribute('href').substring(1);const targetElement=document.getElementById(targetId);const headerOffset=75;const elementPosition=targetElement.getBoundingClientRect().top;const offsetPosition=elementPosition+window.pageYOffset-headerOffset;window.scrollTo({top:offsetPosition,behavior:'smooth'});});});UpdateByStorage();UpdateHeadding();});